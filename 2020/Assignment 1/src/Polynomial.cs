...
public class Polynomial : IDegree
{
    ...
    public void AddTerm(Term t)
    {
        ...
        if (front == null)
        {
            Term item = new Term(t.Coefficient, t.Exponent);
            Node<Term> newNode = new Node<Term>(item, null);
            front = newNode;
            return;
        }
        do
        ...
        } while (comparison < 0);         //Loops while Term t is less than the term being pointed at.

        if (comparison > 0)
        {
            Term item = new Term(t.Coefficient, t.Exponent);
            Node<Term> node = new Node<Term>(item, currentNode);       //When Term t is greater than current Term, create a new node with node.Next = currentNode 
